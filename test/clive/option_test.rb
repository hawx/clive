...
class OptionTest < MiniTest::Unit::TestCase
  ...
  def test_head
    h = Clive::Option.new([:a],nil, nil,{:head => true})
    assert h.head?
    refute h.tail?
  end

  def test_tail
    t = Clive::Option.new([:a],nil, nil,{:tail => true})
    assert t.tail?
    refute t.head?
  end

  def test_parsing_argument_string
    a = Clive::Option.new([:a],nil, nil,{:args => '<h> <w>'})
    b = Clive::Option.new([:a],nil, nil,{:args => '<h> [<w>]'})
    c = Clive::Option.new([:a],nil, nil,{:args => '[<h> <w>]'})
  ...
  def test_parsing_invalid_argument_string
    assert_raises Clive::InvalidArgumentString do
      Clive::Option.new([:a], nil, nil, {:args => 'invalid'})
    end
  end
  ...
  def test_running_with_arguments
    a = nil
    o = Clive::Option.new([:a],nil, nil,{:args => '<h> <w>'}) {|h,w| a = [h,w] }
    o.run({}, [5, 10])
    assert_equal [5,10], a

    o = Clive::Option.new([:a],nil, nil,{:args => '<size>'}) { a = size }
    o.run({}, [50])
    assert_equal 50, a
  end

  def test_default_argument_value
    o = Clive::Option.new([:name], "", nil, {:default => "John"})
    o.run({})
  end

  def test_infer_needs_argument_with_in
    o = Clive::Option.new([:T, :type], "", nil, {:in => %w(small medium large)})
    assert_equal 1, o.args.size
    assert_argument [:arg, false, nil, nil, %w(small medium large)], o.args.first

    o = Clive::Option.new([:T, :type], "", nil, {:in => [%w(small medium large), %w(wide thin)]})
  ...
  def test_infer_needs_argument_with_match
    o = Clive::Option.new([:T, :type], "", nil, {:match => /\d+/})
    assert_equal 1, o.args.size
    assert_argument [:arg, false, nil, /\d+/, nil], o.args.first

    o = Clive::Option.new([:T, :type], "", nil, {:match => [/\d+/, /\d+/, /\d+/]})
  ...
  def test_infer_needs_argument_with_as
    o = Clive::Option.new([:T, :type], "", nil, {:as => Symbol})
    assert_equal 1, o.args.size
    assert_argument [:arg, false, Clive::Type::Symbol, nil, nil], o.args.first

    o = Clive::Option.new([:T, :type], "", nil, {:as => [String, Integer]})
    assert_equal 2, o.args.size
  ...
  def test_infer_needs_argument_with_default
    o = Clive::Option.new([:T, :type], "", nil, {:default => "large"})
    assert_equal 1, o.args.size
    assert_argument [:arg, true, nil, nil, nil], o.args.first

    o = Clive::Option.new([:T, :type], "", nil, {:default => ["large", 5]})
    assert_equal 2, o.args.size
  ...
  def test_infers_arguments_properly_with_multiple_options
    o = Clive::Option.new([:T, :type], "", nil, {:default => "large", :as => Symbol})
    assert_equal 1, o.args.size
  ...
  def test_comparison_with_heads_and_tails_work
    a = Clive::Option.new([:a])
    a_head = Clive::Option.new([:a], '', nil, {:head => true})
    a_tail = Clive::Option.new([:a], '', nil, {:tail => true})
    b = Clive::Option.new([:b], '')
    b_head = Clive::Option.new([:b], '', nil, {:head => true})
    b_tail = Clive::Option.new([:b], '', nil, {:tail => true})
  ...
  def test_can_add_infinite_args
    o = Clive::Option.new([:add], "", nil, {:args => "<item>..."})
